...
const enTozh = require('./enTozh.js')
...
function deepGenerateSidebar(arr) {
  ...
  for (let key in sidebar) {
    ...
    pathPice = pathPice.reduce((pre, cur) => {
      ...
      let enarr = cur[cur.length - 1].replace(/\.md$/, '').split('-')
      let [ idx , entext ] = enarr
      let text = idx + '.' + enTozh[entext] || entext
  ...
  sidebarList[0].chapter.sort((a, b) =>  {
    return parseInt(a.text.split('-')[0]) -parseInt( b.text.split('-')[0])
  })
  return sidebarList
}
